...
- 27.01.26: SHC code moved to a Git submodule (`src`) pointing to https://github.com/apos/SmartHandController (fork) with upstream set to https://github.com/hjd1964/SmartHandController. README updated with submodule workflow.
...
3. [A breakout board for the ESP32](https://www.cyberport.de/tools/search.html?store=ST_DD_WS&query=MacBook+Pro+16%22+2024+M4+Pro%2F24%2F1+TB&page=1) with USB-C (V) and a standard 5/2.1 mm jack (4.5 to 16V) - other breakout boards might fit. But these are very common. 
...
Please consider wleding **right angled pins** (7pin header or 4  header) onto the board (facing inwards) which could give you more space e. g. later on for a battery pack. The even ones delivered with them are not very good for the building into the housing. 
    ...
    Reset  D16 (like in Pin.Esp32.h -> OLED 1309)
